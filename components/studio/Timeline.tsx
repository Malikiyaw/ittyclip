...
export function Timeline() {
  ...
  const bounds = useMemo(() => {
    ...
    for (const h of highlights) { set.add(h.start); set.add(h.end); }
    for (const c of clips) { set.add(c.start); set.add(c.end); }
  ...
  usePlayheadRaf((t) => {
    ...
    if (parent) {
      const pos = t * zoom, left = parent.scrollLeft, right = left + parent.clientWidth;
  ...
  const snap = (v: number) => {
    ...
    for (const b of bounds) { const d = Math.abs(b - v); if (d < bestD) { bestD = d; best = b; } }
    return best;
  };
      ...
      <div ref={scrollRef} className="relative min-w-0 flex-1 touch-pan-x overflow-x-auto overflow-y-hidden">
        <div className="relative h-full" style={{ width: contentWidth }}>
          ...
          <div
            ...
            onPointerDown={(e) => { const rect = e.currentTarget.getBoundingClientRect(); const x = e.clientX - rect.left; useStudio.getState().setPlayhead(clamp(x / zoom)); }}
            role="slider" aria-label="Timeline ruler" aria-valuemin={0} aria-valuemax={Math.round(duration)} aria-valuenow={Math.round(playhead)}
          >
            {ticks.map((t) => <div key={t} className="absolute top-1.5 flex items-start" style={{ left: t * zoom }}><span className="h-2 w-px bg-white/25" /><span className="ml-1.5 font-mono text-[9px] text-white/35">{fmtClock(t)}</span></div>)}
          </div>
          ...
          <div className="absolute top-[86px] bottom-0 left-0" style={{ width: contentWidth }}>
            ...
            {clips.length === 0 && <div className="flex h-full items-center justify-center"><p className="font-mono text-[11px] text-white/30">No clips in timeline — add highlights from the left panel</p></div>}
          </div>

          <div ref={playheadRef} className="pointer-events-none absolute top-0 bottom-0 z-10 w-px bg-white shadow-[0_0_12px_rgba(255,255,255,0.6)]" style={{ transform: `translateX(${playhead * zoom}px)` }}>

...
import { importFromLink, LinkImportError } from "@/lib/importLink";
...
export function UploadZone() {
  ...
  const [drag, setDrag] = useState(false), [fileSizeWarn, setFileSizeWarn] = useState(false), [url, setUrl] = useState(""), [importing, setImporting] = useState(false), [resolving, setResolving] = useState(false), [importProgress, setImportProgress] = useState(0), [importError, setImportError] = useState<LinkImportError | null>(null);
  ...
  const handleImport = async (rawUrl?: string) => {
    ...
    try { await importFromLink(raw, (mb) => { setResolving(false); setImportProgress(mb); }); setUrl(""); }
      ...
      setResolving(false);
      setImportError(err instanceof LinkImportError ? err : new LinkImportError(err instanceof Error ? err.message : "Import failed — try a different link."));
    }
  ...
  const pasteFromClipboard = async () => {
    ...
    try {
      ...
      if (!text.trim()) return showToast("Your clipboard is empty.");
      setUrl(text.trim()); setImportError(null);
      showToast("Link pasted — press Import to continue.");
    } catch {
      showToast("Paste permission was blocked. Tap the field, then use Paste from the iOS keyboard menu.");
    }
  };
  ...
  const openSourceLink = () => {
    ...
    try {
      ...
      showToast("Enter a valid http(s) video link first.");
    }
  };

  const trySample = (e: React.MouseEvent<HTMLButtonElement>) => { e.preventDefault(); e.stopPropagation(); if (busy) return; setUrl(SAMPLE_VIDEO_URL); void handleImport(SAMPLE_VIDEO_URL); };
    ...
    <div className="relative flex flex-1 items-center justify-center overflow-hidden p-8">
      ...
      <div className="relative z-10 w-full max-w-2xl">
        <div role="button" tabIndex={0} aria-label="Upload a video" onClick={(e) => { if (e.target === e.currentTarget && !busy) inputRef.current?.click(); }} onKeyDown={(e) => e.key === "Enter" && !busy && inputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={(e) => { e.preventDefault(); setDrag(false); handleFile(e.dataTransfer.files?.[0]); }} className={`relative flex flex-col items-center justify-center overflow-hidden rounded-[32px] border-2 border-dashed px-8 py-20 text-center transition-all duration-300 ${drag ? "scale-[1.015] border-white/70 bg-white/[0.06] shadow-[0_0_60px_rgba(255,255,255,0.12)]" : "border-white/20 bg-white/[0.03]"} ${busy ? "cursor-wait" : "cursor-pointer"}`}>
          ...
          <button type="button" onClick={trySample} disabled={busy} className="mt-5 rounded-full border border-white/20 px-4 py-2 text-[11px] font-medium text-white/60 transition-colors hover:border-white/50 hover:text-white disabled:cursor-not-allowed disabled:opacity-40">No video handy? Try the sample →</button>
          ...
          {analyzing && <div className="absolute inset-x-0 bottom-0"><div className="border-t border-white/10 bg-black/70 px-7 py-4 text-left backdrop-blur"><div className="mb-2 flex items-center justify-between gap-4"><p className="font-mono text-[11px] tracking-wide text-white/80 uppercase">{stage || "Preparing"} · {Math.round(progress * 100)}%</p><button type="button" onClick={(e) => { e.stopPropagation(); cancelAnalysis(); }} className="rounded-full border border-white/25 px-3 py-1 text-[10px] font-medium text-white/70 transition-colors hover:border-white/60 hover:text-white">Cancel</button></div><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="relative h-full overflow-hidden rounded-full bg-white transition-all duration-300" style={{ width: `${Math.round(progress * 100)}%` }}><span className="absolute inset-0 animate-pulse-soft bg-gradient-to-r from-transparent via-white/70 to-transparent" /></div></div><ol className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">{STEPS.map((label, i) => { const stepIdx = analysisStepIndex(progress), done = i < stepIdx, active = i === stepIdx; return <li key={label} className={`flex items-center gap-1.5 text-[10px] ${done ? "text-white/80" : active ? "text-white/60" : "text-white/30"}`}>{done ? <span className="flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full bg-white text-[8px] font-bold text-black">✓</span> : active ? <span className="h-3.5 w-3.5 shrink-0 animate-pulse rounded-full border border-white/50" /> : <span className="h-3.5 w-3.5 shrink-0 rounded-full border border-white/15" />}{label}</li>; })}</ol></div></div>}
        </div>
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="s-display text-sm uppercase tracking-[0.2em] text-white/80">Or import from a link</h3><span className="s-chip">YouTube · TikTok · Reels · Shorts</span></div><form className="flex items-center gap-2" onClick={(e) => e.stopPropagation()} onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); void handleImport(); }}><input type="url" value={url} onChange={(e) => { setUrl(e.target.value); if (importError) setImportError(null); }} placeholder="Paste a link — YouTube, TikTok, Reels, or direct .mp4" aria-label="Video link" disabled={busy} className="h-11 flex-1 rounded-xl border border-white/15 bg-black/40 px-4 font-mono text-xs text-white placeholder-white/30 outline-none transition-colors focus:border-white/60" /><button type="button" onClick={pasteFromClipboard} disabled={busy} className="h-11 rounded-xl border border-white/15 px-3 text-xs font-medium text-white/70 transition-colors hover:border-white/40 hover:text-white disabled:opacity-40" aria-label="Paste link from clipboard">Paste</button><button type="submit" disabled={busy || !url.trim()} className="h-11 rounded-xl bg-white px-5 text-xs font-semibold tracking-wide text-black transition-opacity hover:opacity-85 disabled:opacity-40">{importing ? "Importing…" : "Import"}</button></form>
          {importError?.code === "PLATFORM_BOT_CHECK" && <div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[0.06] p-4 text-left"><p className="text-xs font-semibold text-white">This video site requires verification</p><p className="mt-1.5 text-[11px] leading-relaxed text-white/55">IttyClip can import public, downloadable media, but it cannot bypass a site's human-verification or login protection.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => inputRef.current?.click()} className="rounded-lg bg-white px-3 py-2 text-[11px] font-semibold text-black">Upload video file</button><button type="button" onClick={openSourceLink} className="rounded-lg border border-white/15 px-3 py-2 text-[11px] text-white/80">Open video site</button><button type="button" onClick={() => void handleImport()} className="rounded-lg border border-white/15 px-3 py-2 text-[11px] text-white/80">Try again</button><button type="button" onClick={() => { setUrl(""); setImportError(null); }} className="rounded-lg border border-white/15 px-3 py-2 text-[11px] text-white/60">Clear</button></div><p className="mt-3 text-[10px] leading-relaxed text-white/35">If the site provides a direct video file URL, paste that URL here instead. IttyClip can then download the media directly.</p></div>}
          {importError && importError.code !== "PLATFORM_BOT_CHECK" && <p className="mt-3 text-[11px] leading-relaxed text-white/60" role="alert">{importError.message}</p>}
          {importing && <div className="mt-4"><div className="mb-1.5 flex items-center justify-between"><p className="font-mono text-[11px] text-white/70">{resolving ? "Resolving link…" : `Downloading video · ${Math.round(importProgress)} MB`}</p><p className="font-mono text-[11px] text-white/40">{resolving ? "resolving" : "downloading"}</p></div><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full animate-pulse rounded-full bg-white" /></div></div>}
        </div>

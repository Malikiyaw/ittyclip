...
import { ANALYSIS_STAGES } from "@/lib/audio";
...
function analysisStepIndex(progress: number): number {
  let idx = 0;
  for (let i = 0; i < ANALYSIS_STAGES.length; i++) {
    if (progress >= (ANALYSIS_STAGES[i][0] as number) - 0.0001) idx = i;
  }
  return idx;
}
...
export function UploadZone() {
  ...
  const trySample = () => {
    ...
    void handleImport(SAMPLE_VIDEO_URL);
  };
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        >
          ...
          <button
            onClick={trySample}
            disabled={busy}
            className="mt-5 rounded-full border border-white/20 px-4 py-2 text-[11px] font-medium text-white/60 transition-colors hover:border-white/50 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
          >
            No video handy? Try the sample →
          </button>

          {analyzing && (
            <div className="absolute inset-x-0 bottom-0">
              <div className="border-t border-white/10 bg-black/70 px-7 py-4 text-left backdrop-blur">
                <div className="mb-2 flex items-center justify-between gap-4">
                  <p className="font-mono text-[11px] tracking-wide text-white/80 uppercase">
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-white/10">
                  <div
                    className="relative h-full overflow-hidden rounded-full bg-white transition-all duration-300"
                    style={{ width: `${Math.round(progress * 100)}%` }}
                  >
                    <span className="absolute inset-0 animate-pulse-soft bg-gradient-to-r from-transparent via-white/70 to-transparent" />
                  </div>
                </div>
                <ol className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">
                  {STEPS.map((label, i) => {
                    const stepIdx = analysisStepIndex(progress);
                    ...
                    return (
                      <li
                        ...
                        className={`flex items-center gap-1.5 text-[10px] ${
                          done ? "text-white/80" : active ? "text-white/60" : "text-white/30"
                        }`}
                      >
                        {done ? (
                          <span className="flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full bg-white text-[8px] font-bold text-black">
                            ✓
                          </span>
                        ) : active ? (
                          <span className="h-3.5 w-3.5 shrink-0 animate-pulse rounded-full border border-white/50" />
                        ) : (
